...
News and Reddit.  A number of commenters raised various objections to the
argument as I presented it, but most of the objections centered around the leap
I made from a computer that manipulates rocks in a field to the randomly
...
each column I place a rock in either the upper row or the bottom row.  If the
rock is in the upper row I consider it a "0" and if it is in the bottom row I
consider it a "1."  These two rows will correspond to the ticker tape of a
Turing machine.
...
machine.  If I have drawn $N$ boxes on the ground, I have an $N$-state Turing
machine.
...
In manipulating these rocks in the desert, I am just acting as a Turing
machine.  By predetermining the right set of state transitions and starting
with the right set of inputs, I can use these rocks to compute any computable
function.  And, in point of fact, if we believe that consciousness is a
...
Presumably this machine is also a computer even though I did not initially
understand the encoding that the machine was using to represent 0s and 1s.
...
Presumably this machine is also a computer even though its encoding at any
particular point in time is random.
